#!/bin/bash
#
# Build the XPS 9315 kernel on the Xeon host, then deploy to the XPS.
#
# Usage:
#   ./build-kernel-remote.sh pull     - Fetch source + config from XPS
#   ./build-kernel-remote.sh build    - Build kernel + modules (make -j32)
#   ./build-kernel-remote.sh deploy   - Push built kernel + modules to XPS
#   ./build-kernel-remote.sh all      - Do all three steps
#
# After deploy, SSH to the XPS and run the install steps printed at the end.
#

set -euo pipefail

# --- Configuration ---
XPS_HOST="user@xps-9315.local"
KVER="6.12.58-gentoo"
...
do_pull() {
    info "Pulling kernel source and .config from XPS..."

    mkdir -p "${BUILD_DIR}"

    # rsync the clean source tree (exclude any prior build artifacts on XPS)
    ...
    rsync -az --delete \
        ...
        --exclude='System.map' \
        --exclude='.config.old' \
        "${XPS_HOST}:/usr/src/linux-${KVER}/" "${SRC_DIR}/"

    # Grab the running .config
    info "Fetching running kernel .config..."
    scp "${XPS_HOST}:/usr/src/linux/.config" "${SRC_DIR}/.config"
...
do_build() {
    [[ -d "${SRC_DIR}" ]] || error "Source dir not found. Run: $0 pull"
    [[ -f "${SRC_DIR}/.config" ]] || error "No .config found. Run: $0 pull"
...
do_deploy() {
    [[ -f "${SRC_DIR}/arch/x86/boot/bzImage" ]] || error "No bzImage found. Run: $0 build"
    ...
    local mod_staging="${BUILD_DIR}/modules-${krelease}"
    rm -rf "${mod_staging}"
    make INSTALL_MOD_PATH="${mod_staging}" modules_install

    info "Deploying to XPS..."

    # Push bzImage
    info "Pushing kernel image..."
    scp arch/x86/boot/bzImage "${XPS_HOST}:/tmp/vmlinuz-${krelease}"

    # Push System.map
    scp System.map "${XPS_HOST}:/tmp/System.map-${krelease}"

    # Push .config for reference
    scp .config "${XPS_HOST}:/tmp/config-${krelease}"

    # Push modules
    info "Pushing modules..."
    rsync -az "${mod_staging}/lib/modules/${krelease}/" \
        "${XPS_HOST}:/tmp/modules-${krelease}/"

    info ""
    info "=== Files staged on XPS in /tmp ==="
    info "  /tmp/vmlinuz-${krelease}"
    info "  /tmp/System.map-${krelease}"
    info "  /tmp/config-${krelease}"
    info "  /tmp/modules-${krelease}/"
    info ""
    info "=== SSH to the XPS and run as root ==="
    cat <<INSTALL
...
case "${1:-}" in
    ...
    all)
        ...
        ;;
    *)
        echo "Usage: $0 {pull|build|deploy|all}"
        echo ""
        echo "  pull    - Fetch kernel source + .config from XPS"
        echo "  build   - Build kernel + modules on this host"
        echo "  deploy  - Push built artifacts to XPS (/tmp)"
