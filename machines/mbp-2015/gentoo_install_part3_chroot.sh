...
# MacBook Pro 12,1 (Early 2015) - Run INSIDE the chroot
# ============================================================================
# This script runs all 13 phases in sequence.
# Goal: ONE chroot session. Everything works on first boot.
#
# Lessons from: MBP 2015 original (6+ re-entries), SP6 gold standard
#
# USAGE:
...
make defconfig
echo "  [OK] defconfig base"

bash "$CONFIGS/kernel_config.sh"
echo "  [OK] MBP 12,1 config applied"
...
# Build kernel (2C/4T Broadwell)
echo "[2.5] Building kernel (make -j5)..."
echo "  This will take 30-60 minutes on 2C/4T Broadwell..."
make -j5
...
# Apple EFI ignores standard EFI boot variables — must use --removable
echo "[3.2] Installing GRUB to EFI (Apple: --removable required)..."
grub-install --target=x86_64-efi --efi-directory=/boot/efi --removable

echo "[3.3] Installing GRUB defaults..."
if [[ -f "$CONFIGS/grub" ]]; then
    cp "$CONFIGS/grub" /etc/default/grub
    echo "  [OK] /etc/default/grub installed (libata.force=noncq, reboot=pci, i915 power)"
else
    echo "  [WARN] No grub defaults found — add boot params manually!"
fi
...
echo "[3.6] Verifying boot params..."
grep -E "libata.force=noncq|reboot=pci" /boot/grub/grub.cfg && \
    echo "  [OK] Boot params present" || \
    echo "  [WARN] Boot params missing — check /etc/default/grub"
...
echo "gentoo-mbp" > /etc/hostname

# Hosts file
cat > /etc/hosts << 'EOF'
127.0.0.1       localhost
::1             localhost
127.0.1.1       gentoo-mbp.localdomain gentoo-mbp
EOF
...
echo ""
echo "[8.3] Verifying critical services..."
...
if [[ -f "$CONFIGS/lightdm.conf" ]]; then
    ...
    echo "  [OK] lightdm.conf installed (no HiDPI — standard resolution)"
else
...
echo ""
echo "  NOTE: CS4208 Apple audio (subsystem 0x106b7b00)"
echo "  If headphone/speaker auto-switching fails after boot, try:"
echo "    echo 'options snd-hda-intel model=mbp11' > /etc/modprobe.d/snd-hda-intel.conf"
...
# ============================================================================
# PHASE 11: APPLE-SPECIFIC HARDWARE
# ============================================================================
echo "=========================================="
echo "=== PHASE 11: Apple Hardware ==="
echo "=========================================="

echo "[11.1] Configuring zram-init..."
cat > /etc/conf.d/zram-init << 'EOF'
# MacBook Pro 12,1: 16GB RAM, use 4GB compressed swap (zstd)
...
echo "[11.2] Running WiFi firmware fix (BCM43602)..."
if [[ -f "$CONFIGS/wifi_firmware_fix.sh" ]]; then
    bash "$CONFIGS/wifi_firmware_fix.sh" || echo "  [WARN] wifi_firmware_fix.sh reported issues (check above)"
    echo "  [OK] WiFi firmware fix script ran"
else
    echo "  [WARN] wifi_firmware_fix.sh not found — check firmware manually"
fi

echo "[11.3] Verifying firmware files..."
ls /lib/firmware/brcm/brcmfmac43602-pcie.bin* 2>/dev/null && echo "  [OK] BCM43602 WiFi firmware" || echo "  [FAIL] BCM43602 WiFi firmware missing!"
ls /lib/firmware/i915/bdw_dmc_ver1_*.bin* 2>/dev/null && echo "  [OK] i915 DMC firmware (Broadwell)" || echo "  [WARN] i915 DMC firmware not found (optional on Broadwell)"

echo "[11.4] Verifying mbpfan config..."
if [[ -f /etc/mbpfan.conf ]]; then
    grep "min_fan1_speed" /etc/mbpfan.conf && echo "  [OK] mbpfan.conf installed"
else
    echo "  [FAIL] /etc/mbpfan.conf missing! Copy from staging."
fi

echo "[11.5] Verifying applesmc module..."
...
# GRUB boot params
grep -q "libata.force=noncq" /boot/grub/grub.cfg 2>/dev/null && echo "[OK] GRUB: libata.force=noncq" || { echo "[FAIL] Missing libata.force=noncq!"; FAIL=$((FAIL+1)); }
grep -q "reboot=pci" /boot/grub/grub.cfg 2>/dev/null && echo "[OK] GRUB: reboot=pci" || { echo "[FAIL] Missing reboot=pci!"; FAIL=$((FAIL+1)); }
...
# Services (mbpfan, NOT thermald/tlp — Apple uses mbpfan)
for svc in dbus NetworkManager display-manager acpid bluetooth mbpfan sshd; do
...
# Firmware
ls /lib/firmware/brcm/brcmfmac43602-pcie.bin* &>/dev/null && echo "[OK] WiFi firmware" || { echo "[FAIL] WiFi firmware!"; FAIL=$((FAIL+1)); }
...
if [[ $FAIL -eq 0 ]]; then
    ...
    echo "  3. Verify fan: cat /sys/devices/platform/applesmc.768/fan1_output"
    echo "  4. Verify zram: swapon --show"
    echo "  5. Verify suspend: echo mem > /sys/power/state (S3 deep)"
    echo "  6. Restore desktop: bash ~/gentoo-machines/shared/restore-desktop.sh"
    echo "  7. Setup hotkeys: bash ~/gentoo-machines/machines/mbp-2015/setup-hotkeys.sh"
else
