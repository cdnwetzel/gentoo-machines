...
## Latest Session: Rename update-kernel.sh → update-system.sh + New Features

### What Was Done
1. **Renamed `update-kernel.sh` → `update-system.sh`**: Script has grown beyond kernel updates — now handles portage sync, @world packages, config file merging, kernel build/install, post-reboot verification, and cleanup. Name now reflects full scope.
2. **Added `eselect news read` to `do_fetch()`**: After portage sync, shows pending Gentoo news items that may announce required USE flag changes or migration steps.
3. **Added `config-update` subcommand**: Runs `dispatch-conf` to merge config file updates generated by emerge. Checks for pending `._cfg????_*` files in `/etc`, reports count, runs dispatch-conf interactively. Supports `--dry-run`.
4. **Integrated `config-update` into `full` workflow**: New phase order: `fetch → world → config-update → check → prepare → build → install → reboot → verify → clean` (10 phases, was 9).
5. **Updated all references across 7 files**: README.md, CLAUDE.md, INSTALL.md, checkpoint.md, backlog.md, shared/machine-checklist.md — zero stale `update-kernel` references remain.
6. **Updated `do_world()` end message**: Now reminds about `config-update` for standalone use.

### Files Modified
- `tools/update-kernel.sh` → `tools/update-system.sh` (git mv + content edits)
- `README.md` — tool name, section header, workflow description, subcommand list
- `CLAUDE.md` — tool name, section header, subcommand list, LTS note
- `INSTALL.md` — tool name, quick reference table, workflow examples
- `checkpoint.md` — all historical references
- `backlog.md` — XPS 9510 migration task
- `shared/machine-checklist.md` — Step 9 kernel update instructions (expanded)

### Verification
- `bash -n tools/update-system.sh` — syntax check passes
- `grep -r 'update-kernel' .` — zero results (all references updated)

---

## Previous Session: eclean-kernel Integration + fetch/clean Subcommands

### What Was Done
1. **Added `fetch` subcommand to `update-system.sh`** (~55 lines): `emerge --sync`, `emerge -v gentoo-sources`, auto-detect newest kernel in `/usr/src/`, `eselect kernel set`. Requires root. `--dry-run` runs `emerge -pv` instead. Idempotent (skips if already selected).
2. **Added `clean` subcommand to `update-system.sh`** (~40 lines): `eclean-kernel -n 3` (keep current + 2 rollback), then `grub-mkconfig` to sync GRUB. Requires root. `--dry-run` maps to `eclean-kernel --pretend`. Shows before/after kernel count.
...
### Files Modified
- `tools/update-system.sh` — fetch + clean subcommands, updated header + usage + case
- `shared/world` — app-admin/eclean-kernel
- `CLAUDE.md` — update-system.sh examples with fetch/clean
...
### Updated Workflow
```
1. sudo update-system.sh fetch       # sync + install + eselect
2. update-system.sh check            # pre-flight
3. update-system.sh prepare          # config migrate + patches
4. update-system.sh build            # compile
5. sudo update-system.sh install     # modules + kernel + NVIDIA
6. reboot
7. update-system.sh verify           # post-reboot checks
8. sudo update-system.sh clean       # eclean-kernel -n 3
```

### Next Session
- XPS 9510: execute 6.12→6.18 migration with `tools/update-system.sh all`
...
### Files Created
- `tools/update-system.sh` — local kernel update tool
...
- `CLAUDE.md` — update-system.sh docs, LTS note, SP6 file table
